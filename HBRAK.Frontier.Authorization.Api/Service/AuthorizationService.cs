...
public class AuthorizationService : IAuthorizationService
{
    ...
    public async Task<ApiToken?> AddTokenFromWebsiteCookie(string token, CancellationToken ct = default)
    {
        _logger.LogInformation("Adding token from website cookie");
        var result = ApiToken.FromWebsiteCookie(token)!;
        Tokens.Add(result);
        await _store.SaveAsync(result, result.ApplicationId, result.EveSub, ct);
        return result;
    }

    public async Task<ApiToken?> AuthorizeAsync(string redirectUri, IEnumerable<string> scopes, CancellationToken ct = default)
    {
        
        _logger.LogCritical("Authorization via browser is not implemented yet by CCP.");
        return await Task.FromResult<ApiToken?>(null); //this is not implemented yet on CCP side :(
    }

    public Task<IReadOnlyList<ApiToken>> LoadAllAsync(CancellationToken ct = default)
        => _store.LoadAllAsync<ApiToken>(_options.Value.AppId, ct);
    ...
    public async Task LoadAndRefreshAllAsync(CancellationToken ct = default)
    {
        ...
        Tokens.AddRange(await Task.WhenAll(
            savedTokens.Select(async t =>
            {
                ...
                await _store.SaveAsync(refreshed, refreshed.ApplicationId, refreshed.EveSub, ct);
                return refreshed;
            })));
    }

    public async Task<ApiToken> RefreshIfNeededAsync(ApiToken token, CancellationToken ct = default)
    {
    ...
    public async Task<ApiToken> RefreshAsync(ApiToken token, CancellationToken ct)
    {
        ...
        ApiToken result = ApiToken.FromRefreshResponse(json)!;
