...
namespace HBRAK.Frontier.Authorization.Api.Data;
...
public class ApiToken
{
    ...
    [JsonPropertyName("eve_sub")] public required string EveSub { get; set; }
    ...
    public static ApiToken? FromToken(string? token)
    {
        if (string.IsNullOrWhiteSpace(token)) return null;

        var payloadJson = GetPayloadJson(token);
        var claims = JsonSerializer.Deserialize<ApiToken>(payloadJson)!;
        claims.Token = token;
        return claims;
    }

    public static ApiToken? FromWebsiteCookie(string jwt)
    {
        ...
        ApiToken? token = FromToken(accessCode) ?? null;
    ...
    public static ApiToken? FromRefreshResponse(string response)
    {
    ...
    public static string GetPayloadJson(string jwt)
    {
        ...
        return Encoding.UTF8.GetString(Decode(parts[1]));
    }
}
