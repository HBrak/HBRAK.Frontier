...
public class EvmChainContractsService : IChainContracts
{
                                               ...
                                               string blockTag = "latest", CancellationToken ct = default)
    {
        ...
        {
            var (name, sel, sig) = ErrorDecoder.TryDecodeName(abiJson, scEx.ExceptionEncodedData);
       ...
       CancellationToken ct = default)
    {
        ...
        var callArgs = NormalizeNumericArgs(inputParams, args ?? []);
        ...
        {
            var (name, sel, sig) = ErrorDecoder.TryDecodeName(worldAbiJson, scEx.ExceptionEncodedData);
    ...
    private static object[] NormalizeNumericArgs(Parameter[] pars, object[] provided)
    {
        ...
        for (int i = 0; i < provided.Length; i++)
        {
            ...
            {
                var v = provided[i];
                if (v is HexBigInteger h) { outArgs[i] = h.Value; continue; }
